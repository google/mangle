...
func TestTemporalStore_CoalesceMixedGranularity(t *testing.T) {
	store := NewTemporalStore()

	atom := ast.NewAtom("event", name("/sensor"))

	// Second-level interval
	store.Add(atom, ast.TimeInterval(
		time.Date(2024, 1, 1, 10, 0, 0, 0, time.UTC),
		time.Date(2024, 1, 1, 10, 0, 5, 0, time.UTC),
	))
	// Overlapping millisecond-level interval
	store.Add(atom, ast.TimeInterval(
		time.Date(2024, 1, 1, 10, 0, 4, 500000000, time.UTC),
		time.Date(2024, 1, 1, 10, 0, 6, 0, time.UTC),
	))
	// Adjacent nanosecond-level interval
	store.Add(atom, ast.TimeInterval(
		time.Date(2024, 1, 1, 10, 0, 6, 1, time.UTC),
		time.Date(2024, 1, 1, 10, 0, 7, 0, time.UTC),
	))

	pred := ast.PredicateSym{Symbol: "event", Arity: 1}
	if err := store.Coalesce(pred); err != nil {
		t.Fatalf("Coalesce error: %v", err)
	}

	// Should be coalesced into 1 interval from 10:00:00 to 10:00:07
	if store.EstimateFactCount() != 1 {
		t.Errorf("After coalesce: count = %d, want 1, got %d", store.EstimateFactCount(), store.EstimateFactCount())
	}

	query := ast.NewQuery(pred)
	store.GetAllFacts(query, func(tf TemporalFact) error {
		expectedStart := time.Date(2024, 1, 1, 10, 0, 0, 0, time.UTC)
		expectedEnd := time.Date(2024, 1, 1, 10, 0, 7, 0, time.UTC)
		if !tf.Interval.Start.Time().UTC().Equal(expectedStart) {
			t.Errorf("Coalesced start = %v, want %v", tf.Interval.Start.Time().UTC(), expectedStart)
		}
		if !tf.Interval.End.Time().UTC().Equal(expectedEnd) {
			t.Errorf("Coalesced end = %v, want %v", tf.Interval.End.Time().UTC(), expectedEnd)
		}
		return nil
	})
}
