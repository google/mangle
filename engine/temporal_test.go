	...
	tests := []struct {
	...
	}{
		{
			...
			operator: ast.TemporalOperator{
				...
				Interval: ast.NewInterval(
					ast.NewDurationBound(0),               // 0 duration
					ast.NewDurationBound(30*24*time.Hour), // 30 days
				),
			},
			wantSolns:   1,
		...
		},
		{
			...
			operator: ast.TemporalOperator{
				Type: ast.DiamondMinus,
				Interval: ast.NewInterval(
					ast.NewDurationBound(0), // 0 days ago
					ast.NewDurationBound(3*24*time.Hour),
				),
			},
			wantSolns:   0,
	...
	tests := []struct {
	...
	}{
		{
			...
			operator: ast.TemporalOperator{
				Type: ast.BoxMinus,
				Interval: ast.NewInterval(
					ast.NewDurationBound(0),
					ast.NewDurationBound(30*24*time.Hour),
				),
			},
			wantSolns:   1,
		...
		},
		{
			...
			operator: ast.TemporalOperator{
				...
				Interval: ast.NewInterval(
					ast.NewDurationBound(0),
					ast.NewDurationBound(60*24*time.Hour),
				),
			},
			wantSolns:   0,
...
func TestTemporalEvaluator_EternalFact(t *testing.T) {
	...
	operator := ast.TemporalOperator{
		...
		Interval: ast.NewInterval(
			ast.NewDurationBound(0),
			ast.NewDurationBound(365*24*time.Hour),
		),
	}
	...
	tests := []struct {
		...
		},
		{
			...
			operator: ast.TemporalOperator{
				...
				Interval: ast.NewInterval(
					ast.NewDurationBound(0),
					ast.NewDurationBound(5*24*time.Hour),
				),
			},
			wantSolns:   0,
...
func TestTemporalEvaluator_IntervalVariableBinding(t *testing.T) {
	...
	// Query with interval variable binding @[T1, T2]
	t1Var := ast.Variable{Symbol: "T1"}
	t2Var := ast.Variable{Symbol: "T2"}
	interval := ast.NewInterval(ast.NewVariableBound(t1Var), ast.NewVariableBound(t2Var))
	...
	// Check that the interval variables were bound
	boundStart := solutions[0].Get(t1Var)
	if boundStart == nil {
		t.Error("Start variable T1 was not bound")
	}
	boundEnd := solutions[0].Get(t2Var)
	if boundEnd == nil {
		t.Error("End variable T2 was not bound")
	}

	// Check values
	if c, ok := boundStart.(ast.Constant); ok {
		if c.NumValue != startTime.UnixNano() {
			t.Errorf("T1 = %v, want %v", c.NumValue, startTime.UnixNano())
		}
	} else {
		t.Errorf("Expected constant for T1, got %T", boundStart)
	}

	if c, ok := boundEnd.(ast.Constant); ok {
		if c.NumValue != endTime.UnixNano() {
			t.Errorf("T2 = %v, want %v", c.NumValue, endTime.UnixNano())
		}
	} else {
		t.Errorf("Expected constant for T2, got %T", boundEnd)
	}
}
