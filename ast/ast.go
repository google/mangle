...
func AddVars(term Term, m map[Variable]bool) {
	...
	case TemporalLiteral:
		AddVars(t.Literal, m)
		if t.Interval != nil {
			addVarsFromInterval(*t.Interval, m)
		}
		if t.Operator != nil {
			addVarsFromInterval(t.Operator.Interval, m)
		}
	case TemporalAtom:
		AddVars(t.Atom, m)
		if t.Interval != nil {
...
func addVarsFromInterval(interval Interval, m map[Variable]bool) {
	if interval.Start.Type == VariableBound {
		m[interval.Start.Variable] = true
	}
	if interval.End.Type == VariableBound {
