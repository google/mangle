...
> [!IMPORTANT]
> If a predicate is declared `temporal`, **all** its facts and rules must have a temporal annotation (`@[...]`). Unannotated facts are not allowed for temporal predicates. Conversely,non-temporal predicates cannot have temporal annotations.
