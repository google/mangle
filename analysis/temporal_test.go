...
func TestCheckTemporalRecursion(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			program: `
				...
				derived(X) @[_, _] :- derived(X) @[_,_], some_condition(X).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantSeverity == SeverityCritical {
				if err == nil {
					t.Fatal("expected analysis error for critical warning, got nil")
				}
				if !strings.Contains(err.Error(), tt.wantMsgContains) {
					t.Errorf("error %q does not contain %q", err, tt.wantMsgContains)
				}
				return
			}
			if err != nil {
			...
			// For non-critical warnings, we still manually call CheckTemporalRecursion
			// to verify them, as Analyze logs them but doesn't return them.
